...
public class ItemDbRepository {
    ...
    public long createItem(String title, String body, String colorHex) {
        String now = Instant.now().toString();
        ContentValues values = new ContentValues();
        values.put("title", title.trim());
        values.put("body", emptyToNull(body));
        values.put("color_hex", emptyToNull(colorHex));
        values.put("created_at", now);
        values.put("updated_at", now);
        return db().insertOrThrow("items", null, values);
    }

    public void updateItem(long itemId, String title, String body, String colorHex) {
        ContentValues values = new ContentValues();
        values.put("title", title.trim());
        values.put("body", emptyToNull(body));
        values.put("color_hex", emptyToNull(colorHex));
    ...
    public List<ItemRecord> listItems(String query) {
        ...
        Cursor c = db().rawQuery(
                "SELECT i.id, i.title, i.body, i.color_hex, i.created_at, i.updated_at, " +
        ...
        while (c.moveToNext()) {
            ...
            item.colorHex = c.getString(3);
            item.createdAt = c.getString(4);
            item.updatedAt = c.getString(5);
            item.rowCount = c.getInt(6);
            item.topicCount = c.getInt(7);
            result.add(item);
        }
        c.close();
        return result;
    ...
    public List<ItemRecord> listItemsFiltered(String query, String tagsText, String locationText, String colorHex, boolean matchAllTags) {
        List<ItemRecord> base = listItems(query);
        List<String> tags = parseFilters(tagsText);
        String location = locationText == null ? "" : locationText.trim();
        String wantedColor = colorHex == null ? "" : colorHex.trim();
        ArrayList<ItemRecord> filtered = new ArrayList<>();
        for (ItemRecord item : base) {
            if (!wantedColor.isEmpty()) {
                String itemColor = item.colorHex == null ? "" : item.colorHex.trim();
                if (!wantedColor.equalsIgnoreCase(itemColor)) continue;
            }
            ItemDetail detail = getItemDetail(item.id);
            if (!matchesTags(detail, tags, matchAllTags)) continue;
            if (!location.isEmpty() && !matchesLocation(detail, location)) continue;
            filtered.add(item);
        }
        return filtered;
    }
    ...
    public ItemDetail getItemDetail(long itemId) {
        ...
        Cursor itemCursor = db().rawQuery(
                "SELECT id, title, body, color_hex, created_at, updated_at FROM items WHERE id = ?",
        ...
        if (itemCursor.moveToFirst()) {
            ...
            detail.item.colorHex = itemCursor.getString(3);
            detail.item.createdAt = itemCursor.getString(4);
            detail.item.updatedAt = itemCursor.getString(5);
        }
        itemCursor.close();
    ...
    public void addRow(long itemId, String priceText, String location, String entryDate, String ranking, String progressText) {
        ContentValues values = buildRowValues(itemId, priceText, location, entryDate, ranking, progressText);
        values.put("sort_order", nextSortOrder("item_rows", itemId));
        db().insertOrThrow("item_rows", null, values);
        touchItem(itemId);
    }

    public void updateRow(long rowId, String priceText, String location, String entryDate, String ranking, String progressText) {
        long itemId = parentItemId("item_rows", rowId);
        ContentValues values = buildRowValues(itemId, priceText, location, entryDate, ranking, progressText);
        db().update("item_rows", values, "id = ?", new String[]{String.valueOf(rowId)});
        if (itemId > 0) touchItem(itemId);
    }

    public void addTag(long itemId, String tag) {
    ...
    public void deleteTag(long entryId) {
    ...
    public String buildAnalysisReport(String tagsText, String locationText, boolean matchAllTags) {
        ...
        out.append("Tag mode: ").append(matchAllTags ? "All selected tags" : "Any selected tag").append("\n");
        ...
        for (ItemRecord item : items) {
            ...
            for (ItemRowEntry row : detail.rows) {
                if (location.isEmpty() || location.equalsIgnoreCase(safe(row.location))) rows.add(row);
            }
            if (!location.isEmpty() && rows.isEmpty()) continue;
            ...
            if (!rows.isEmpty()) {
                ...
                for (ItemRowEntry row : rows) {
                    if (row.progressText != null && !row.progressText.trim().isEmpty()) rowsWithProgress++;
                    out.append("    - ")
    ...
    public String exportJsonToFile() throws Exception {
        ...
        root.put("item_tags", queryArray("SELECT id, item_id, topic, sort_order FROM item_topics ORDER BY item_id, sort_order, id"));
    ...
    public String previewImport(String json) throws Exception {
        ...
        return "Import preview ready. items=" + length(parsed, "items") +
                ...
                " tags=" + Math.max(length(parsed, "item_tags"), length(parsed, "item_topics"));
    }
    ...
    public void importJson(String json, boolean replaceFirst) throws Exception {
        ...
        try {
            ...
            insertArray(db, parsed.optJSONArray("items"), "INSERT OR IGNORE INTO items(id, title, body, color_hex, created_at, updated_at) VALUES(?, ?, ?, ?, ?, ?)", new String[]{"id", "title", "body", "color_hex", "created_at", "updated_at"});
            ...
            JSONArray tags = parsed.optJSONArray("item_tags");
            if (tags == null) tags = parsed.optJSONArray("item_topics");
            insertArray(db, tags, "INSERT OR IGNORE INTO item_topics(id, item_id, topic, sort_order) VALUES(?, ?, ?, ?)", new String[]{"id", "item_id", "topic", "sort_order"});
    ...
    private ContentValues buildRowValues(long itemId, String priceText, String location, String entryDate, String ranking, String progressText) {
        ContentValues values = new ContentValues();
    ...
    private List<NamedEntry> readNamedEntries(String table, String valueColumn, long itemId) {
        ...
        Cursor c = db().rawQuery("SELECT id, " + valueColumn + ", sort_order FROM " + table + " WHERE item_id = ? ORDER BY sort_order, id", new String[]{String.valueOf(itemId)});
        while (c.moveToNext()) {
    ...
    private boolean matchesTags(ItemDetail detail, List<String> filters, boolean matchAll) {
        if (filters.isEmpty()) return true;
        ArrayList<String> itemTags = new ArrayList<>();
        for (NamedEntry entry : detail.topics) itemTags.add(entry.value == null ? "" : entry.value.trim().toLowerCase(Locale.US));
        if (matchAll) {
            for (String filter : filters) {
                if (!itemTags.contains(filter.toLowerCase(Locale.US))) return false;
            }
            return true;
        }
        for (String filter : filters) {
    ...
    private boolean matchesLocation(ItemDetail detail, String location) {
        for (ItemRowEntry row : detail.rows) {
            if (location.equalsIgnoreCase(safe(row.location))) return true;
        }
        return false;
    }
    ...
    private String joinValues(List<NamedEntry> entries) {
        ...
        for (NamedEntry entry : entries) values.add(safe(entry.value));
